...
//
// 1. COLUMN-BASED LAYOUT: The diagram uses fixed x-coordinates (0, 2, 4, 5, 6)
//    to create distinct columns. This is ideal for neural network architectures.
//
// 2. VARIABLES FOR SPACING: All spacing values are defined at the top:
//    `let node-spacing = 2`, `let layer-spacing = 2`
//    This makes the diagram easily resizable.
//
// 3. NAMED ANCHORS: Every node has a `name: "xxx"` for connection points.
//    The pattern `"arrow1.east"` connects to `"attn1"` automatically.
//
// 4. BEZIER FOR LOOPS: The f_psi connections use bezier curves instead of
//    straight lines. The control point is calculated as midpoint - 2 units,
//    creating a smooth arc effect.
//
// 5. STYLING CONSISTENCY: `set-style` at the top applies to all elements.
//    Arrow style is defined once: `(end: "stealth", fill: black, scale: 0.7)`
//
// 6. CONTENT FRAMES: `frame: "rect"` for data nodes, `frame: "circle"`
//    for operations (like multiplication ×). This visual distinction helps
//    readers understand the diagram semantics.
//
// WHY THIS APPROACH WORKS:
// - Absolute x-coordinates + relative y-coordinates = predictable layout
// - No anchor references would make lines misaligned
// - Using variables instead of magic numbers = maintainable
